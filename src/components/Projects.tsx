...
const projects = [
  ...
  impact: [
    'Simplified access to complex ocean data',
    'Enabled natural language querying',
    'Centralized data exploration platform'
  ],
  icon: Brain,
  link: null,
  image: 'media/floatchat.jpg', // put your image in public folder
},
{
    id: 4,
  ...
  impact: [
    'Showcases technical and creative work in one place',
    'Optimized UI for clarity and engagement',
    'Responsive and performance-focused design'
  ],
  icon: Briefcase,
  link: null, // add your live link later
  image: 'media/port.png', // add image in public folder
},
{
  id: 9,
  ...
  icon: Briefcase, // we’ll fix import below
  link: null,
  image: 'media/coming.png',
},
...
/**
 * IMAGE SIDE-LAYOUT — matches the target diagram:
 *
 *   [IMG][  CARD (left col)  ]    [  CARD (right col)  ][IMG]
 *
 * Implementation:
 *   • Each grid cell is `position: relative; overflow: visible`
 *   • The image is `position: absolute` pinned to the outer edge of the cell
 *   • The card has a CSS-variable-driven margin that pushes it inward by the image width
 *   • On mobile (<768 px) the image is hidden and margin resets to 0
 */
export const Projects = () => {
  const { ref, isVisible } = useScrollAnimation(0.1);

  // Image strip width as % of the grid column
      ...
      <div ref={ref} className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-9">
          {projects.map((project, index) => {
            ...
            // Even index → left column → image on LEFT, card shifts right
            // Odd index  → right column → image on RIGHT, card shifts left
            ...
            return (
              ...
              <div
                key={project.id}
                className={`group relative w-full transition-all duration-600 ${
              ...
              >
                <img
                  ...
                  alt=""
                  aria-hidden="true"
                  className="hidden md:block absolute top-1/2 -translate-y-1/2 h-[90%] w-[350px] object-cover rounded-lg opacity-60 blur-[0px] z-0 pointer-events-none select-none drop-shadow-[0px_0px_10px_rgba(255,0,0,0.6)]"
                  style={{
                    
                    [isLeftCol ? 'left' : 'right']: '-250px',
                ...
                {/*
                  ...
                  - Hidden on mobile (< md).
                  - Absolutely pinned to the outer edge of the grid column.
                  - z-0 so the card (z-10) always renders on top.
                */}
                

                {/*
                  Card.
                  On desktop: pushed inward by IMG_W so the thumbnail strip is fully exposed beside it.
                  On mobile:  no margin (thumbnail is hidden anyway).
                */}
                <div
                  className={`relative z-10 h-full flex flex-col bg-[#0F172A] rounded-xl border-t-4 ${config.borderColor} border-x border-b border-gray-800 overflow-hidden hover:-translate-y-1 hover:border-gray-700 transition-all duration-300`}
                  
                >
                  {/*
                    Inline responsive style: apply margin only at md+ breakpoint.
                    A single scoped rule per card is fine — they share the same value.
                  */}
                  

                  {/* data attr used by the scoped style above */}
                  <div data-card-id={project.id} className="flex flex-col flex-1 p-6 gap-4">
                    ...
                    {/* Description */}
                    <p
                      className="text-1xl md:text-1xl text-gray-400 text-base leading-relaxed line-clamp-2"
